...
List installed skills with `/skills` (project `.claude/skills/` + global `~/.claude/skills/`; `user-invocable: false` entries show `[auto-only]`). Load one with `/skill-name`: its instructions join the conversation and its `allowed-tools` are pre-approved for that turn only. Relevant skills also auto-load on description match — never re-announce them, just follow them. When a loaded skill mentions `references/<file>` or `scripts/<file>`, that file's content arrives inlined with the skill; anything else under the skill dir is not visible unless read with a file tool.
