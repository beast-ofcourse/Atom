...
function highlightFamilyFor(
      ...
      if (
            /^(sh|bash|zsh|shell|console|terminal|dotenv|env|bashrc|zshrc)$/.test(
                  l,
            )
      )
            return "sh";
      if (
            /^(json|jsonc|yaml|yml|toml|ini|xml|html|css|scss|less|graphql|gql|sql|md|markdown)$/.test(
                  l,
            )
      )
            return "data";
            ...
            <Box flexDirection="column" marginTop={gap ? 1 : 0}>
                  {lang ? (
                        <Box flexDirection="row">
                              ...
                              <Text dimColor>
                                    {" "}
                                    {theme.symbol.rule.repeat(
                                          Math.min(
                                                24,
                                                Math.max(4, 30 - lang.length),
                                          ),
                                    )}
                              </Text>
                        </Box>
