...
// Slash-menu engine lives in ui/slash-menu.ts (deep module): registry +
// matchers + menu builders. Re-exported here so existing `../src/App.js`
// imports keep working.
export {
  SLASH_COMMANDS,
  SLASH_MENU_SKILL_CAP,
  SKILL_MENU_DESC_CHARS,
  buildSlashMenu,
  filterSkillPicker,
  filterSlashCommands,
  fuzzyScore,
...
export const GOAL_USAGE =
  "usage: /goal <objective> (set and start working, just like a normal message; replacing resets counters; mid-turn set replaces quietly) · /goal (show with cumulative stats) · /goal pause · /goal resume (re-arms; idle starts a turn, busy resumes at turn end) · /goal clear (ends it) · model tools mirror the slash (get_goal reads; create_goal on /goal intent only, never inferred; pause/resume/clear_goal in matching state; complete only on evidence)";
        ...
        historyRef.current,
        {
          ...
          turnEvents: {
            onToolStarted: (info) => {
              ...
              try {
                stepBlocksRef.current = startToolBlock(stepBlocksRef.current, {
                  step: liveStepRef.current,
                  hint: info.name,
                });
                flushDraft();
              } catch {
                // ignore (the legacy tool-hint lane still shows the call)
              }
            },
            onToolFinished: (info) => {
              try {
          ...
          onToolActivity: (label, result, isError) => {
            ...
            stepBlocksRef.current = completeToolBlock(stepBlocksRef.current, {
              ...
              durationMs: ms,
              summary,
              errorLine: isError ? errorFirstLine : null,
            });
            const items: Turn[] = [];
            ...
            const toolTurn: Turn = { role: "tool", content: label, ms, summary };
            items.push(toolTurn);
            ...
            if (isError) {
              ...
              // (flush also carries the done flip — the ref flipped above).
              flushDraft();
              items.push({
                role: "tool",
                content: `  ${theme.symbol.detailMark} ${errorFirstLine}`,
                error: true,
              });
