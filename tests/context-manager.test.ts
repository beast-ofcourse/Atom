// ContextManager tests: window-derived budgets, usage answers, and
// compaction decisions. History is uncapped — there is no trim step, so
// nothing here drops turns. Hermetic ATOM_HOME (temp dir) so no real
// atom.json can leak in; env vars saved/restored per file.
...
describe("budget derivation", () => {
  test("1M-window model gets a window-derived allowance", async () => {
    await isolateHome();
  ...
  test("unknown-window model reports no allowance (never invented)", async () => {
    await isolateHome();
  ...
  test("budget carries no caps: nothing to enforce, nothing to trim to", async () => {
    await isolateHome();
    const mgr = createContextManager({ model: "deepseek-v4-pro", toolsChars: 0 });
    const b = mgr.budget(sysHistory(0)) as Record<string, unknown>;
    expect(b).not.toHaveProperty("effectiveMaxChars");
    expect(b).not.toHaveProperty("effectiveMaxMessages");
    expect(b).not.toHaveProperty("hardCeilingChars");
    expect(mgr).not.toHaveProperty("trimForSend");
  });
});
